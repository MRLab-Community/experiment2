...
if (!class_exists('FPSM_qqUploadedFileXhr')) {

    /**
     * Handle file uploads via XMLHttpRequest
     */
    class FPSM_qqUploadedFileXhr {
    ...
    class FPSM_qqFileUploaders {
        ...
        function __construct(array $allowedExtensions = array(), $sizeLimit = 10485760) {
            ...
            if (isset($_GET['qqfile'])) {
                $this->file = new FPSM_qqUploadedFileXhr();
            } elseif (isset($_FILES['qqfile'])) {
                $this->file = new FPSM_qqUploadedFileForm();
            } else {
                $this->file = false;
            }
        }
        ...
        function handleUpload($uploadDirectory, $replaceOldFile, $upload_url) {
            ...
            if (!$this->file) {
                return array('error' => esc_html__('No files were uploaded.', 'frontend-post-submission-manager'));
            }

            $size = $this->file->getSize();

            if ($size == 0) {
                return array('error' => esc_html__('File is empty', 'frontend-post-submission-manager'));
            }
